...
{% for mirror, opts in salt['pillar.get']('aptly:mirrors').items() %}
  ...
  {%- if opts['url'] is defined -%}
    ...
    {%- set create_mirror_cmd = "aptly mirror create --ignore-signatures -architectures='" ~ opts['architectures']|default([])|join(',') ~ "' " ~ mirror ~ " " ~ opts['url'] ~ " " ~ opts['distribution']|default('') ~ " " ~ opts['components']|default([])|join(' ') -%}
